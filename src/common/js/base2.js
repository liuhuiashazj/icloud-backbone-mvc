...
require(['jquery'], function ($) {
    ...
    var AjaxObj = {
        ...
        request: function (options) {
            var url = options.url, type = options.type,isFullUrl=url.indexOf('?')==-1?0:1,//url中含有?的情况处理
            self = this, param,
            t = new Date().getTime(),
            ...
            if (type == 'GET' && data) url = url + (isFullUrl?'&':'?') + param;
    ...
    var Doms = {
        ...
        getParentByTag: function (el, name) {
            var parent = el.parentNode, key, type = 1, cls;
            if (name.match(/^\./)) {
                name = name.replace(/^\./, '');
            ...
            function match(node, name, type) {
                var reg;
                if (type) {
                    return node['tagName'].toLowerCase() == name;
                } else {
                    //reg=new RegExp('(\\s+'+name+'\\s+)|(^'+name+'\\s*)|(\\s*'+name+'$)');
                    cls = ' ' + node['className'] + ' ';
                    name = ' ' + name + ' ';
                    return cls.indexOf(name) == -1 ? 0 : 1;
                }
            }

            while (!match(parent, name, type) && parent.tagName != 'BODY') {
