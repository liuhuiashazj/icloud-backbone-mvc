...
<%if(data.showWrap&&data.docs.length){%>
    ...
    <tr class="htr">
        <th class="t1"><input type="checkbox" class="selectall bluebox"/><%if(data.route=='team'&&0){%><span class="sort">文件<span class="by <%=data.sort.by||"desc"%>"  data-sort="fname"></span></span><%}else{%>文件<%}%></th>
        <th class="t2"></th>
        <th class="t3"></th>
        <%if(data.route=='search'){%><th class="t6 categoryh">来源</th><%}%>
        <th class="t4"><%if(data.route=='team'&&0){%><span class="sort">更新时间<span class="by <%=data.sort.by||"desc"%>"  data-sort="ftime"></span></span><%}else{%>更新时间<%}%></th>
        <%if(data.fromShare&&data.route=='ibox'){%><th class="t6">上传者</th><%}%>
        <th class="t5"><%if(data.route=='team'&&0){%><span class="sort">大小<span class="by <%=data.sort.by||"desc"%>"  data-sort="fsize"></span></span><%}else{%>大小<%}%></th>
...
<%for(var i=0,l=data.docs.length;i<l;i++){ var doc=data.docs[i];%>
    ...
    >
        ...
        <td>
            ...
            <%if(data.route=='ibox'&&doc.isdir!='1'){%><span class="share" data-tips="转发到hi"></span><%}%>
